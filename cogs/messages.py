...
from services.pubsub import fetch_sub_of_the_week, get_sub
...
class Messages(commands.Cog, name='messages'):
    ...
    async def list_commands(self, ctx: Context) -> None:
        command_list = []
        command_embed = Embed(title="RMY Bot Commands", color=Color.red(), description="Here's a list of my commands:")
        for command in self.bot.commands:
            command_list.append((command.name, command.description))

        command_list.sort()
        for command in command_list:
            command_embed.add_field(name=command[0], value=command[1], inline=False)
    ...
    # pubsub crashes bot and hangs. Need to update async when fixed.
    ##################################################################################################################
    #@commands.hybrid_command(name='pubsub', description='Posts the pubsub of the week.')
    #async def pubsub(self, ctx: Context, update: str = None) -> None:
    #    sub_response = await fetch_sub_of_the_week() if (update and update.lower() == 'update') else await get_sub()
    #    if sub_response:           
    #        await ctx.send(f"This week's Publix Sub is the {sub_response}!")
